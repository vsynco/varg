...
async function mostrarPuig(req, res, pageTitle) {
  ...
  async function mostrarFotosDrive(req, res, pageTitle) {
    try {
      const id = req.params.id;
      const pageToken = req.query.pageToken || "";
      const contacto = await accesosModel.mostrarContactoNatural(id);
      ...
      async function listPhotos(pageToken) {
        try {
          const response = await drive.files.list({
            pageSize: 100,
            pageToken: pageToken,
          ...
          if (error.response && error.response.status === 401) {
            ...
            await persona.save();
            oauth2Client.setCredentials(tokens);
            return listPhotos(pageToken);
          }
          throw error;
        }
      }
  
      await listPhotos(pageToken);
  
    } catch (error) {
      console.error("Error general:", error);
      res.status(500).send("Hubo un error al cargar las fotos de Drive");
    }
  }
  ...
  async function mostrarVideosDrive(req, res, pageTitle) {
    try {
      const id = req.params.id;
      const pageToken = req.query.pageToken || "";
      const contacto = await accesosModel.mostrarContactoNatural(id);
      ...
      async function listVideos(pageToken) {
        try {
          const response = await drive.files.list({
            pageSize: 100,
            pageToken: pageToken,
          ...
          if (error.response && error.response.status === 401) {
            ...
            await persona.save();
            oauth2Client.setCredentials(tokens);
            return listVideos(pageToken);
          }
          throw error;
        }
      }
  
      await listVideos(pageToken);
  
    } catch (error) {
      console.error("Error general:", error);
      res.status(500).send("Hubo un error al cargar los videos de Drive");
    }
  }
